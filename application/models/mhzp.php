...
// 			}
	...
	public function hzp(){
		...
		while($i >=0){
			...
			$this->dbs->select('*')->from('hzp');
			//$this->dbs->where('class_3' , 880);
			$this->dbs->where('id > ' , 93052);
			$this->dbs->limit(1,$i);
			...
			if(!empty($res)){//919459
				...
				$brands		= $this->_get_brand_info($res[0]['brand']);
				$data['brand_id'] = false == $brands ? 0 : $brands;
		
				$goods_id	= createGoodsId('',$classes['class_id'],'h');
				$data['goods_id']	= $goods_id;
				$data['param4']	= $data['grand_id'] . $data['parent_id'] . $data['class_id'] . $data['brand_id'];
				//print_r($data);exit;
				...
				$this->dbs->insert('up_hzp_goods' , $data);
				$this->dbs->insert('up_hzp_goods_param' , $param_data);
				echo $res[0]['id'] . "\n";
			}else{
				$i = -1;
			}
	...
	private function _get_class_info($class_3 = 0){
		...
		$this->dbs->select('*')->from('hzp_class_3');
